...
public class ReservationDao extends AbstractDao<Reservation, Long> {
	...
	public List<Reservation> getReservationByUser(Long userId, int limit) {
		Query selectQuery = entityManager()
				.createQuery("SELECT r FROM Reservation r JOIN r.user m WHERE m.id=:userID ORDER BY r.reservationStart DESC")
	...
	public List<Reservation> getReservationByDateAndResource(Long resourceID, Date startDate, Date stopDate) {
		Query selectQuery = entityManager().createQuery("SELECT DISTINCT r FROM Reservation r LEFT JOIN r.resource res WHERE "//
		...
		//Quick fix : WHERE clause on resourceID doesn't work here, so remove unwanted result
		//TODO : fix this to have a correct request
		List<Reservation> resultList = selectQuery.getResultList();
		Iterator<Reservation> iterator = resultList.iterator();
		while (iterator.hasNext()) {
			Reservation reser = iterator.next();
			Resource resource = reser.getResource();
	...
	public List<Reservation> getReservationByDateAndResourceExcludePrevious(Long previousReservationId, Long resourceID, Date startDate,
			Date stopDate) {
		Query selectQuery = entityManager().createQuery("SELECT r FROM Reservation r JOIN r.resource res WHERE "//
				+ "res.id=:resourceID AND "//
	...
	public Long getNbReservationByUser(Long id) {
		return (Long) entityManager()
				.createQuery(
						"select count(r) from Reservation r join r.user m where " + "m.id = :userId and " + "r.reservationStop >= current_date()")
